...
export const NodePreview: React.FC<NodePreviewProps> = ({ node, onClick, buttonText }) => {
  return (
    <Card className="my-2 bg-background/80 border-border/60 shadow-sm hover:shadow-lg transition-shadow">
      <CardHeader className="py-1.5 border-b flex flex-row justify-between items-center flex-nowrap">
        <span className="text-base font-medium text-foreground truncate">{node.name}</span>
        <Button size="sm" variant="outline" className="" onClick={onClick}>
          {buttonText}
        </Button>
      </CardHeader>
      <CardContent>
        <div className="my-2 p-0 rounded [mask-image:linear-gradient(to_top,transparent,black_50%)]">
          <div className="w-[280px] h-[120px] overflow-hidden relative">
            <div
